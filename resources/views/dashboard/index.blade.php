...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <a href="/purchase-orders/create" class="flex items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
            ...
            <a href="/stock-opnames/create" class="flex items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
...
async function loadDashboard() {
    try {
        ...
        // Load statistics
        const [itemsRes, requestsRes, poRes] = await Promise.all([
            window.api.get('/items?per_page=1'),
            window.api.get('/requests?status=submitted&per_page=1'),
            window.api.get('/purchase-orders?status=pending_approval&per_page=1')
        ]);

        // Update statistics
        const totalItems = itemsRes.data?.data?.total || itemsRes.data?.data?.data?.length || 0;
        const pendingRequests = requestsRes.data?.data?.total || requestsRes.data?.data?.data?.length || 0;
        ...
        const recentRes = await window.api.get('/requests?per_page=5');
        ...
        // Show error in recent requests section
